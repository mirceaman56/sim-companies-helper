(() => {
  ...
  async function loadAuthDataOnce() {
    console.log("[SCX] loadAuthDataOnce called", {
...
function getRowFromTarget(target) {
  ...
  // Dark theme row wrapper (bootstrap-ish)
  const dark = target.closest('div.row.css-1jgxqiq.e1bf4c274');
  if (dark && dark.querySelector('input[name="price"]') && dark.querySelector('input[name="quantity"]')) {
    return dark;
  }

  // Old wrapper (light theme)
  const light = target.closest("div.css-mv4qyq");
  if (light) return light;

  // Generic fallback: nearest ancestor that contains both inputs
  let el = target;
...
function getProductNameFromRow(row) {
  ...
  // ---------- init ----------
  console.log("[SCX] init start");
  ensureSidebar();
  (async () => {
  console.log("[SCX] init IIFE running");
  await loadAuthDataOnce();
  console.log("[SCX] auth after load", JSON.parse(JSON.stringify(STATE.auth)));
  await loadInventoryOnce();
  console.log("[SCX] inventory after load", {
    status: STATE.inventory.status,
    loaded: STATE.inventory.loaded,
    itemsLen: STATE.inventory.items?.length,
    byKindSize: STATE.inventory.byKind?.size,
    error: STATE.inventory.error,
  });
  updatePanel();  
  console.log("[SCX] updatePanel called from init");
})();
